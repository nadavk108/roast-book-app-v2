...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const { data: books, error } = await supabaseAdmin
      .from('roast_books')
      .select('id, created_at, victim_name, status, slug, cover_image_url, lemonsqueezy_order_id, quotes, preview_image_urls, full_image_urls, user_email, video_status, video_url')
    ...
    const recentBooks = allBooks.slice(0, 25).map(b => ({
      ...
      hasPaid: b.lemonsqueezy_order_id !== null,
