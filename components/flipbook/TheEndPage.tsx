...
export function TheEndPage({ victimName, bookUrl, bookId }: TheEndPageProps) {
  ...
  const handleWhatsAppShare = () => {
    try { captureEvent(Events.SHARE_BUTTON_CLICKED, { book_id: bookId, share_method: 'whatsapp' }); } catch {}
    const text = encodeURIComponent(`${shareText}\n\n${bookUrl}`);
    window.open(`https://wa.me/?text=${text}`, '_blank');
  };

  const handleSMSShare = () => {
    try { captureEvent(Events.SHARE_BUTTON_CLICKED, { book_id: bookId, share_method: 'sms' }); } catch {}
    const text = encodeURIComponent(`${shareText} ${bookUrl}`);
    window.open(`sms:?body=${text}`, '_blank');
  };

  const handleEmailShare = () => {
    try { captureEvent(Events.SHARE_BUTTON_CLICKED, { book_id: bookId, share_method: 'email' }); } catch {}
  ...
  return (
    <div className="w-full h-full bg-gradient-to-br from-gray-900 via-black to-gray-900 flex flex-col items-center justify-center p-6 md:p-8 text-center relative overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-sm space-y-6">
        ...
        {/* CTA - prominent, above attribution */}
        <div className="pt-5 border-t border-white/20">
          <p className="text-lg font-heading font-black text-white mb-1">
